# รายงานการวิเคราะห์และแผนการปรับปรุงบทที่ 6: เทคโนโลยีไร้สายสำหรับ IoT (รอบที่ 2)

## 1. บทสรุปการวิเคราะห์และแผนการปรับปรุง (Executive Summary & Plan)

จากการรีวิวเนื้อหาเดิมในไฟล์ `/chapters/ch06-wireless/README.md` ร่วมกับรายงานของผู้ประเมินทั้งสองคน (Reviewer 1 และ Reviewer 2 Reports) พบประเด็นความไม่ถูกต้องทางเทคนิค ข้อบกพร่องในโค้ดตัวอย่างที่อาจทำให้ระบบล่ม และการแปลคำศัพท์ที่คลาดเคลื่อน เพื่อเตรียมความพร้อมให้เอกสารบทนี้มีความสมบูรณ์เชิงวิศวกรรมสำหรับนักศึกษา จึงได้สรุปแผนการแก้ไขออกเป็น 3 กลุ่มดังนี้:

### 1.1 การปรับปรุงเสถียรภาพและคุณภาพโค้ด C++ (ESP32 Wi-Fi / BLE)
1. **ป้องกัน Task Watchdog Reset ใน Wi-Fi Web Server Loop**: เพิ่มการคืนทรัพยากร CPU (Yielding) ด้วยคำสั่ง `delay(1);` ในบล็อก `else` เมื่อ `client.available()` เป็นเท็จ เพื่อป้องกันไม่ให้ CPU วนซ้ำในลูปแบบ Tight Loop นานจนกระทั่ง Task Watchdog Timer (TWDT) สั่งรีเซ็ตบอร์ด
2. **ประกาศตัวแปรแชร์สถานะ BLE เป็น `volatile`**: ป้องกันไม่ให้ Compiler ทำการเก็บค่าตัวแปร `isClientConnected` และ `wasClientConnected` ไว้ใน register ของ CPU ในการรัน `loop()` ซึ่งอาจทำให้ไม่เห็นค่าที่เปลี่ยนไปจริงใน BLE interrupt/task thread อื่น
3. **การหลีกเลี่ยง Serial Print ใน BLE Callbacks**: เนื่องจาก BLE callback ทำงานบน Context ของ BLE Stack Task การทำ Blocking I/O อย่างการสั่งพิมพ์ข้อมูลผ่าน Serial ใน callback ส่งผลให้ระบบเสี่ยงต่อการเกิด packet loss หรือ watchdog reset จึงย้ายการประมวลผลและการดีบักออกไปใน `loop()` โดยควบคุมด้วยแฟล็กแทน
4. **แก้ไขคำสั่ง BLE Configuration ซ้ำซ้อน**: เปลี่ยนจาก `setMinPreferred(0x12);` เป็น `setMaxPreferred(0x12);` เพื่อแก้ไขบั๊กการเรียกฟังก์ชันซ้ำที่เขียนทับกัน
5. **เพิ่ม `public:` ใน BLE Callbacks**: ประกาศขอบเขตระดับการเข้าถึง virtual methods ของ callback class ให้เป็นแบบ `public` อย่างชัดเจนเพื่อให้สอดคล้องกับโครงสร้าง C++

### 1.2 การปรับแก้ข้อมูลทางวิทยาศาสตร์และการสื่อสารไร้สาย (Physics & Wireless Theories)
1. **แก้ไขความเข้าใจผิดเรื่องการดูดซับคลื่น 2.4 GHz ของน้ำ**: ชี้แจงข้อเท็จจริงว่าการดูดซับพลังงานเกิดจากกลไกหมุนตัวทางไฟฟ้าเคมี (Dipole Rotation / Dielectric Relaxation) ไม่ใช่เกิดจากคลื่นไปเรโซแนนซ์กับน้ำ (เนื่องจาก Resonance frequency ของน้ำจริง ๆ อยู่ในย่าน THz)
2. **ปรับสูตรการคำนวณ FSPL (Free Space Path Loss)**: นิยามสูตร FSPL ให้เป็นความสูญเสียในเส้นทางระหว่างสายอากาศแบบ Isotropic ในอุดมคติ (ไม่มี Antenna Gain $G_t, G_r$ มาร่วมในสมการของตัวมันเอง) แล้วจึงนำเสนอสมการงบประมาณลิงก์ (Link Budget / Friis Transmission) แยกต่างหากเพื่อหากำลังสัญญาณฝั่งรับ
3. **แก้ไขทฤษฎี Spreading Factor (SF) ของ LoRa**: อธิบายให้ถูกต้องว่า SF12 จะให้ระดับความไวของฝั่งรับสูงที่สุด (Highest Receiver Sensitivity) ทำให้ถอดรหัสสัญญาณที่ระดับ SNR ติดลบมากๆ ได้ดีที่สุด แต่กินความเร็วและพลังงานสูง ในขณะที่ SF7 ต้องการระดับสัญญาณที่แรงกว่า (ต้องการ SNR สูงกว่าในการถอดรหัส)

### 1.3 การแก้ไขคำศัพท์และคำแปลภาษาไทย (Terminology Corrections)
1. **อักษรวิญญาณ** (Ghost Character) -> เปลี่ยนเป็น **Characteristic** หรือ **คุณลักษณะ**
2. **การเดินเรือข้อมูล** (Data Sailing) -> เปลี่ยนเป็น **การหาเส้นทางข้อมูล (Routing)**
3. **ค่าความปลอดภัย** (Safety Factor) ใน NB-IoT -> เปลี่ยนเป็น **การขยายขีดความสามารถการครอบคลุม (Coverage Enhancement)**
4. **ความเร็วเครื่อง** (Machine Speed) ใน ADR -> เปลี่ยนเป็น **อัตราการส่งข้อมูลวิทยุ (Data Rate)**

---

## 2. เนื้อหาบทที่ 6 ที่ผ่านการปรับปรุงสมบูรณ์ (Revised Chapter 6 Contents)

เนื้อหาด้านล่างนี้ผ่านการตรวจสอบและแก้ไขประเด็นทั้งหมดแล้ว พร้อมสำหรับการนำไปเขียนทับลงในไฟล์ `/chapters/ch06-wireless/README.md` ทันที:
...
*   **ย่าน 2.4 GHz (ความถี่ 2.400 GHz ถึง 2.4835 GHz):**
    ...
    *   *ลักษณะเด่น:* มีแบนด์วิดท์ที่กว้างกว่า ส่งข้อมูลได้เร็วกว่าย่าน Sub-GHz แต่ความยาวคลื่นมีขนาดสั้น (~12.5 cm) ทำให้ถูกดูดซับโดยสิ่งกีดขวางได้ง่าย โดยเฉพาะน้ำและความชื้น (เนื่องจากโมเลกุลของน้ำเป็นโมเลกุลมีขั้ว จึงดูดซับพลังงานจากคลื่นความถี่ในย่านนี้ได้ดีผ่านกลไกการหมุนตัวทางไฟฟ้าเคมี หรือ Dielectric Relaxation/Dipole Rotation ไม่ใช่เกิดจากความถี่เรโซแนนซ์ตามความเข้าใจผิดทั่วไป ซึ่งความถี่เรโซแนนซ์ธรรมชาติของโมเลกุลน้ำอิสระจริง ๆ อยู่ในย่าน THz หรือราว 22 GHz สำหรับไอน้ำ) อีกทั้งยังมีปัญหาการรบกวนของสัญญาณที่แออัดเนื่องจากมีอุปกรณ์ใช้งานหนาแน่น
...
ความสูญเสียในเส้นทางฟรีสเปซ (FSPL) คือค่าความสูญเสียหรือการลดทอนของกำลังสัญญาณวิทยุเมื่อเดินทางผ่านช่องทางที่เป็นอวกาศหรืออากาศที่ไม่มีสิ่งกีดขวาง โดยคิดคำนวณระหว่างสายอากาศแบบรอบทิศทางในอุดมคติ (Isotropic Antennas) ที่มีกำลังขยายเป็น 0 dBi สูตรทางคณิตศาสตร์คำนวณได้ดังนี้:
...
หากแทนค่าความเร็วแสง $c \approx 3 \times 10^8\text{ m/s}$ จะได้สูตรแบบง่ายเมื่อระยะทาง $d$ เป็นเมตร (m) และความถี่ $f$ เป็นเฮิรตซ์ (Hz):
...
ในการคำนวณกำลังสัญญาณจริงที่ฝั่งรับเพื่อวิเคราะห์คุณภาพของระบบสื่อสาร จะใช้ **สมการส่งผ่านของฟรีส (Friis Transmission Equation)** หรือการคำนวณ **งบประมาณลิงก์ (Link Budget)** โดยการคิดรวมกำลังส่งและกำลังขยายของสายอากาศฝั่งส่งและรับเข้ามาด้วย ดังนี้:
$$P_r\text{ (dBm)} = P_t\text{ (dBm)} + G_t\text{ (dBi)} + G_r\text{ (dBi)} - \text{FSPL (dB)}$$
โดยที่:
*   $P_r$ คือ กำลังสัญญาณที่ได้รับที่ฝั่งรับ (Received Power) มีหน่วยเป็น dBm
*   $P_t$ คือ กำลังส่งของเครื่องส่ง (Transmit Power) มีหน่วยเป็น dBm
*   $G_t$ คือ กำลังขยายของสายอากาศฝั่งส่ง (Transmit Antenna Gain) มีหน่วยเป็น dBi
*   $G_r$ คือ กำลังขยายของสายอากาศฝั่งรับ (Receive Antenna Gain) มีหน่วยเป็น dBi
...
void loop() {
  ...
  // 2. ตรวจสอบการร้องขอจากผู้ใช้หรือ SCADA (HTTP Client) เข้ามาที่ Web Server (เฉพาะเมื่อสถานะ Wi-Fi เชื่อมต่อสำเร็จ)
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (client) {
      ...
      while (client.connected() && (millis() - clientConnectedTime < clientTimeoutLimit)) {
        if (client.available()) {
          ...
          delay(1); // ยอมให้งานอื่นใน FreeRTOS และ Idle Task ทำงานเพื่อป้อนสัญญาณเลี้ยง Task Watchdog Timer (TWDT)
        }
      }
...
*   **Characteristic (คุณลักษณะ):** เป็นที่เก็บบันทึกข้อมูลดิบจริง ๆ ของเซ็นเซอร์หรือคำสั่ง มีองค์ประกอบย่อยคือ:
...
// ตัวแปรแชร์สถานะการเชื่อมต่อ (ต้องประกาศ volatile เพื่อป้องกัน register caching จาก Compiler)
volatile bool isClientConnected = false;
volatile bool wasClientConnected = false;

// ตัวแปรและแฟล็กสำหรับการรับคำสั่งเพื่อนำไปดีบักใน loop() ป้องกันการบล็อก BLE stack task ด้วย Serial print
...
// Callback สำหรับรับรู้การต่อเข้า/หลุดออกของ Client (ระบุ public:)
...
// Callback สำหรับรอรับการเขียนข้อมูลเปลี่ยนสถานะ (Write) จาก Client (ระบุ public:)
class CommandReceiverCallbacks: public BLECharacteristicCallbacks {
  public:
    void onWrite(BLECharacteristic *pCharacteristic) {
      std::string receivedData = pCharacteristic->getValue();
      if (receivedData.length() > 0 && receivedData.length() < sizeof(rxCommand)) {
        strcpy(rxCommand, receivedData.c_str());
...
void setup() {
  ...
  // กำหนดขีดจำกัดเกณฑ์เชื่อมต่อ ป้องกันปัญหาการสื่อสารกับบางอุปกรณ์ เช่น iOS
  pAdvertising->setMinPreferred(0x06); 
  pAdvertising->setMaxPreferred(0x12); // แก้ไขข้อผิดพลาดจากเดิมที่เป็นการเรียกฟังก์ชันซ้ำซ้อน
...
void loop() {
  ...
  // 1. ตรวจสอบและประมวลผลคำสั่งจาก BLE Callback (ย้ายมาประมวลผลภายนอก BLE Task Context เพื่อป้องกันการบล็อกระบบ)
  if (commandReceived) {
    Serial.println("\n----------------------------------------");
    ...
    // เปรียบเทียบคำสั่ง
    if (strcmp(rxCommand, "ACTIVATE") == 0) {
      Serial.println("[BLE Control] ผลลัพธ์: เปิดทำงานระบบวาล์วน้ำหลัก");
    } else if (strcmp(rxCommand, "DEACTIVATE") == 0) {
    ...
    Serial.println("----------------------------------------");
    commandReceived = false; // เคลียร์แฟล็กหลังจากประมวลผลสำเร็จ
  }

  // 2. หาก Client ต่ออยู่ ให้อัปเดตค่าอุณหภูมิเป็นระยะๆ
  if (isClientConnected) {
    if (now - prevNotifyTime >= notifyPeriod) {
      ...
      // ส่งข้อมูลด้วยขนาด 2 ไบต์ทาง Characteristic
  ...
  // 3. จัดการแจ้งเตือนเมื่อเกิดการเชื่อมต่อสำเร็จใน loop แทน callback
  ...
  // 4. กลไกจัดการเริ่มสัญญาณ Advertising ใหม่เมื่อ Client ตัดสัมพันธ์ (Auto-Restart Advertising)
  if (!isClientConnected && wasClientConnected) {
...
In ระบบเครือข่ายของ Zigbee หนึ่งเครือข่ายจะแบ่งหน้าที่ของแต่ละอุปกรณ์ออกเป็น 3 บทบาทหลัก:
...
2.  **Zigbee Router (ZR - ตัวสลับและส่งต่อ):**
    *   ทำหน้าที่เป็นตัวผ่านกระจายสัญญาณและหาเส้นทางข้อมูล (Routing) เพื่อส่งข้ามทอดไปยังเครื่องอื่น
...
*   **Spreading Factor (SF):** ค่าปัจจัยการคูณขยายความกว้างของรหัส (มีค่าตั้งแต่ SF7 ถึง SF12)
    *   *SF ต่ำ (SF7):* ข้อมูลส่งได้รวดเร็ว ใช้เวลาอยู่ในอากาศสั้น (Short Time-on-Air) แต่อุปกรณ์รับต้องการระดับสัญญาณที่แรงกว่า (ต้องการค่า SNR ที่สูงกว่าในการถอดรหัส) เหมาะสำหรับระยะใกล้
    *   *SF สูง (SF12):* ข้อมูลเดินทางได้ระยะไกลมาก ทนทานสูงมาก มีความไวต่อสัญญาณสูงที่สุด (สามารถถอดรหัสสัญญาณที่อ่อนมากภายใต้ค่า SNR ที่ติดลบได้ดีที่สุด) แต่ความเร็วข้อมูลช้ามาก (Long Time-on-Air) และส่งผลให้มีการสิ้นเปลืองพลังงานแบตเตอรี่ในอุปกรณ์มากกว่า
...
3.  **Network Server (แม่ข่ายควบคุมระบบ):** ทำหน้าที่ส่วนการคำนวณถอดรหัส ตรวจสอบความปลอดภัย ขจัดข้อมูลซ้ำซ้อนในกรณีมีเกตเวย์หลายตัวได้รับสัญญาณพร้อมกัน และควบคุมอัตราการส่งข้อมูลและกำลังส่งแบบปรับตัว (Adaptive Data Rate - ADR) เพื่อช่วยประหยัดพลังงานแบตเตอรี่
...
*   **กลไกความแรงและการเจาะทะลวงของคลื่น (Deep Indoor Coverage):** มีขีดความสามารถในการรับสัญญาณที่สูงขึ้น หรือการขยายขีดความสามารถการครอบคลุม (Coverage Enhancement) ทำให้รองรับการลดทอนของสัญญาณได้ดีขึ้นสูงสุดถึง +20 dB เมื่อเทียบกับเครือข่ายเซลลูลาร์ทั่วไป จึงเป็นหนึ่งในทางเลือกสำหรับงานอ่านข้อมูลในชั้นใต้ดิน ผนังหนา หรือท่อน้ำประปา
