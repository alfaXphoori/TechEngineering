# รายงานบทวิเคราะห์แผนการขยายเนื้อหาบทที่ 6 และการแก้ไขข้อผิดพลาด (Chapter 6 Revision & Analysis Report)

**ผู้จัดทำ**: Explorer 3 (Chapter 6, Round 2)
**วันที่**: 22 มิถุนายน 2569 (2026-06-22)

---

## 1. ตารางสรุปการแก้ไขข้อผิดพลาดทางเทคนิคและการแปลภาษา (Summary of Key Technical and Translation Resolutions)

ในการประเมินรอบที่ 2 นี้ ได้มีการระบุข้อผิดพลาดทางเทคนิคและข้อบกพร่องด้านการแปลภาษาในเนื้อหาเดิม ซึ่งได้รับการวิเคราะห์หาสาเหตุและปรับปรุงแก้ไขดังรายละเอียดในตารางนี้:

| หัวข้อ / ประเด็นปัญหา | จุดที่พบใน README.md เดิม | สาเหตุเชิงเทคนิค (Root Cause Analysis) | แนวทางแก้ไขและปรับปรุงใหม่ (Resolution) |
|---|---|---|---|
| **1. Wi-Fi Web Server Loop Watchdog Reset** | บรรทัดที่ 250-301 ใน `loop()` | เมื่อไคลเอนต์เชื่อมต่อเข้ามาแต่ยังไม่ส่งข้อมูล โค้ดจะวนลูปอย่างรวดเร็วโดยไม่มีการหน่วงเวลา ทำให้ CPU แกนหลักเกิดภาวะตึงตัวและ starve คิวงานอื่น ส่งผลให้ Task Watchdog Timer (TWDT) ของ FreeRTOS สั่งรีเซ็ตบอร์ด | เพิ่มคำสั่ง `else { delay(1); }` ด้านในลูป `while (client.connected())` เมื่อไม่มีข้อมูลพร้อมอ่าน (`client.available()` เป็นเท็จ) เพื่อคืนเวลาให้แก่ระบบปฏิบัติการ FreeRTOS และระบบเฝ้าระวังบอร์ดไม่ให้ค้าง |
| **2. Wi-Fi Connection Check** | บรรทัดที่ 243 ใน `loop()` | บอร์ดจะสแกนหาไคลเอนต์ด้วยคำสั่ง `WiFiClient client = server.available();` ตลอดเวลาแม้ว่าการเชื่อมต่อวิทยุวิถี Wi-Fi จะหลุดไปแล้ว ซึ่งเป็นการสิ้นเปลืองพลังงานและอาจก่อปัญหาในสแต็ก TCP | ทำการหุ้ม (Wrap) คำสั่งตรวจจับไคลเอนต์ด้วยการตรวจสอบสถานะเชื่อมต่อ `if (WiFi.status() == WL_CONNECTED)` |
| **3. BLE Volatile Variables** | บรรทัดที่ 405-406 ในส่วนประกาศตัวแปร | ตัวแปรสถานะ `isClientConnected` และ `wasClientConnected` ถูกเขียนจาก BLE callbacks (ซึ่งทำงานอยู่บนอีกโปรเซส/คอร์นึงของ FreeRTOS) และถูกอ่านในลูปหลัก หากคอมไพเลอร์ทำ optimization อาจทำการแคชค่าไว้ในรีจิสเตอร์ทำให้ไม่สะท้อนค่าจริงในลูป | ประกาศตัวแปรเชื่อมโยงข้อมูลข้ามเธรดทั้งหมดให้เป็น `volatile` เพื่อบังคับให้คอมไพเลอร์อ่านค่าจากหน่วยความจำหลักโดยตรงเสมอ เช่น `volatile bool isClientConnected = false;` |
| **4. BLE Callback Blocking Prints** | บรรทัดที่ 411-447 ใน Callback classes | การเรียกใช้คำสั่ง `Serial.println()` ภายใน callback task ของ BLE สแต็กถือเป็น anti-pattern เพราะการพิมพ์ผ่านสายส่งซีเรียลเป็นการบล็อก ซึ่งอาจส่งผลให้สแต็กการเชื่อมต่อวิทยุเกิดความหน่วง แพ็กเกจตกหล่น หรือค้างได้ | ย้ายคำสั่งดีบัก `Serial` ทั้งหมดออกจากคลาส Callback แล้วนำมาตรวจสอบและสั่งแสดงผลในลูปหลักของโปรแกรมโดยใช้ธงสถานะ (Status Flags) ร่วมกับตัวแปรเก็บคำสั่งที่ประกาศเป็น `volatile` |
| **5. BLE Callback Access Specifier** | บรรทัดที่ 411-447 ใน Callback classes | คลาสตัวแทนสืบทอดไม่ได้ระบุคำสั่งควบคุมการเข้าถึง `public:` ไว้หน้าฟังก์ชัน ทำให้ฟังก์ชันในสืบทอดถูกกำหนดเป็น `private` โดยปริยายตามคุณสมบัติของ C++ class แม้การทำงานทั่วไปจะทำได้ผ่านพอยเตอร์ของคลาสแม่ แต่จัดเป็นรูปแบบที่ไม่ชัดเจนสำหรับนักศึกษา | กำหนดคำสำคัญ `public:` ให้ชัดเจนในทุกคลาสย่อยของ BLE Callback |
| **6. BLE Advertising Redundant Settings** | บรรทัดที่ 493-494 | มีการเรียกใช้ฟังก์ชัน `setMinPreferred` ซ้อนกันสองบรรทัดด้วยค่าอาร์กิวเมนต์ที่ต่างกัน ทำให้บรรทัดแรกถูกเขียนทับและไม่มีผล | ปรับปรุงคำสั่งที่สองให้เป็น `setMaxPreferred(0x12)` เพื่อตั้งค่าช่วงรอบเวลาการโฆษณาที่เหมาะสมที่สุด |
| **7. การแปลคำว่า Characteristic/Value ผิดพลาด** | บรรทัดที่ 512 | คำว่า "Characteristic" ในโครงสร้าง GATT ถูกแปลผิดเป็น "อักษรวิญญาณ" (Spiritual Character) ซึ่งเกิดจากระบบแปลคำศัพท์ผิดพลาดอย่างรุนแรง | เปลี่ยนการแปลเป็นคำศัพท์เทคนิคมาตรฐานคือ "Characteristic" หรือ "คุณลักษณะ" ในภาษาไทย |
| **8. ความเข้าใจคลาดเคลื่อนด้านฟิสิกส์ เรื่อง Resonance ของน้ำ** | บรรทัดที่ 32 ในหัวข้อย่าน 2.4 GHz | ข้อความระบุว่าความถี่ 2.4 GHz ถูกน้ำดูดซับเนื่องจากเป็นความถี่สั่นพ้อง (Resonance frequency) ของน้ำ ซึ่งในทางฟิสิกส์ความถี่สั่นพ้องของน้ำสูงกว่านี้มากในระดับ THz/GHz การอุ่นอาหารหรือการสูญเสียกำลังในน้ำเกิดจากกลไกโมเลกุลมีขั้วหมุนตามคลื่นวิทยุ | อธิบายกลไกทางฟิสิกส์ให้ถูกต้องว่าเกิดจาก **การผ่อนคลายไดอิเล็กทริก (Dielectric Relaxation)** หรือ **การหมุนตัวทางไฟฟ้าเคมี (Dipole Rotation)** ของโมเลกุลน้ำมีขั้ว ไม่ใช่การสั่นพ้อง |
| **9. นิยามสูตรคำนวณ FSPL ไม่บริสุทธิ์** | บรรทัดที่ 57 ในส่วนอธิบาย FSPL | สมการ FSPL เดิมมีการลบด้วย $G_t$ และ $G_r$ (กำลังขยายสายอากาศส่งและรับ) ซึ่งผิดหลักการคำนวณค่าสูญเสียในสเปซอิสระ ซึ่งคิดจากจุดรับส่งแบบ Isotropic (กำลังขยาย 0 dBi) | แยกส่วนประกอบ $G_t$ และ $G_r$ ออกจากสูตร FSPL อย่างเด็ดขาด และนำเสนอสมการงบประมาณลิงก์ (Link Budget / Received Power) แยกออกมาต่างหากเพื่อให้ถูกต้องตามหลักวิศวกรรมโทรคมนาคม |
| **10. การหาเส้นทางข้อมูล (Routing) ใน Zigbee** | บรรทัดที่ 577 | แปลคำว่า Routing ในอุปกรณ์จัดส่งข้อมูลในเครือข่ายเป็น "การเดินเรือข้อมูล" (Data sailing) ซึ่งเป็นคำแปลภาษาไทยที่ผิด | ปรับเปลี่ยนเป็น "การหาเส้นทางข้อมูล (Routing)" หรือ "การกำหนดเส้นทางและส่งต่อข้อมูล" |
| **11. การขยายสัญญาณใน NB-IoT (Coverage Enhancement)** | บรรทัดที่ 669 | แปลคำว่า Coverage Enhancement หรือ Link Margin เป็น "ค่าความปลอดภัย" (Safety factor) ซึ่งทำให้เกิดความเข้าใจผิดเชิงนิยามวิศวกรรม | เปลี่ยนคำอธิบายเป็น "การขยายขีดความสามารถการครอบคลุมสัญญาณ (Coverage Enhancement)" หรือ "ค่าเผื่อของลิงก์ (Link Margin)" |
| **12. ความไวภาครับและความสัมพันธ์กับ Spreading Factor** | บรรทัดที่ 609-610 | ข้อความระบุว่าที่ SF7 ตัวรับจะต้องไวต่อสัญญาณสูง ซึ่งเป็นเรื่องเข้าใจผิด ในความเป็นจริง SF12 คือจุดที่ตัวรับมีความไวสูงสุด (Sensitivity สูงสุด สามารถถอดรหัสสัญญาณจมคลื่นรบกวนได้) ส่วน SF7 ต้องการสัญญาณแรงกว่า | ปรับปรุงคำอธิบายให้ถูกต้องตามความจริงของ Semtech LoRa: SF12 ให้ความไวภาครับสูงสุด (ถอดรหัสได้ที่ SNR ต่ำสุดถึง -20 dB) ขณะที่ SF7 ต้องการความแรงสัญญาณสูงกว่าในการถอดรหัส |
| **13. อัตราความเร็วข้อมูลแบบปรับตัว (Adaptive Data Rate - ADR)** | บรรทัดที่ 630 | แปลคำว่า Data Rate เป็น "ความเร็วเครื่อง" (Machine speed) | ปรับปรุงเป็น "อัตราการส่งข้อมูลไร้สายแบบปรับตัว" หรือ "อัตราการส่งข้อมูลและกำลังส่งแบบปรับตัว (Adaptive Data Rate - ADR)" |

---

## 2. การวิเคราะห์เชิงลึกและการตรวจสอบความถูกต้องของโค้ด C++ (Deep Technical Analysis & Verification of Revisions)

### 2.1 สถาปัตยกรรม Non-blocking Wi-Fi Server และการรับมือข้อผิดพลาด Watchdog
ในการเขียนโค้ดสำหรับ ESP32 ภายใต้เฟรมเวิร์ก Arduino นั้น โดยพื้นฐานจะมีระบบตรวจจับเวลาการประมวลผล (Task Watchdog Timer) ทำงานอยู่ที่ Core 1 เพื่อป้องกันไม่ให้โค้ดส่วนใดส่วนหนึ่งเข้าสู่ลูปไม่รู้จบ (Infinite Loop) หรือทำการคำนวณโดยไม่แบ่งปันรอบการประมวลผลให้งานระดับระบบอื่น ๆ 
ในระบบควบคุมอุณหภูมิมอเตอร์หรือ SCADA การค้างอยู่ใน `while(client.connected())` นาน 2.5 วินาทีโดยไม่มีคอลแบ็กเพื่อคืนสิทธิ์ส่งผลให้บอร์ดเกิด watchdog reset และเริ่มระบบใหม่ทั้งหมด ซึ่งส่งผลเสียอย่างร้ายแรงต่อการผลิต 

การแก้ไขทำได้โดยใช้รูปแบบ:
```cpp
while (client.connected() && (millis() - clientConnectedTime < clientTimeoutLimit)) {
  if (client.available()) {
    ...
    delay(1); // คืนสิทธิ์การเข้าถึงซีพียูให้แก่ FreeRTOS เพื่อป้องกัน Watchdog รีเซ็ตบอร์ด
  }
}
```
การหน่วงเวลาเพียง `delay(1)` จะทำหน้าที่เรียกใช้ตัวจัดตารางงาน (Scheduler) ของ FreeRTOS ให้นำงานที่มีลำดับความสำคัญเท่ากันหรือน้อยกว่า (เช่น งานป้อน Watchdog หรือสแต็กการจัดการคลื่นวิทยุระดับต่ำ) ขึ้นมาประมวลผลในช่วงจังหวะเสี้ยววินาทีนั้น ทำให้ระบบโดยรวมมีเสถียรภาพสูงมาก

### 2.2 การสื่อสารต่างเธรดในสแต็ก BLE (BLE Multithreading Safety)
สแต็ก Bluetooth ของ ESP32 ทำงานอยู่บน FreeRTOS Task แยกต่างหากซึ่งมักรันอยู่บน Core 0 ของหน่วยประมวลผล ในขณะที่การทำงานของโปรแกรมหลัก `loop()` จะรันอยู่ที่ Core 1 ดังนั้นการรับส่งตัวแปรสถานะ เช่น `isClientConnected` จึงเป็นการสื่อสารแบบข้ามแกนประมวลผล (Inter-core communication) 
หากคอมไพเลอร์ที่ใช้การปรับแต่งระดับสูงมองไม่เห็นการเปลี่ยนแปลงค่าของตัวแปรภายในฟังก์ชัน `loop()` เอง มันจะทำการดึงค่าตัวแปรไปเก็บไว้ในรีจิสเตอร์ของซีพียูถาวรเพื่อลดการเข้าถึง RAM (Register Caching Optimization) ทำให้บอร์ดไม่สามารถตรวจจับการหลุดหรือเชื่อมต่อของอุปกรณ์บลูทูธได้เลย 
การประกาศ `volatile` จะแก้ปัญหานี้โดยตรง:
```cpp
volatile bool isClientConnected = false;
volatile bool wasClientConnected = false;
volatile bool commandReceived = false;
```
นอกจากนี้ การสั่งพิมพ์ข้อมูลดีบักผ่านพอร์ตซีเรียลอุตสาหกรรมในฟังก์ชัน Callback ซึ่งทำงานภายใต้ขอบเขตงานของสแต็กวิทยุ BLE โดยตรง อาจไปสร้างความล่าช้าในระดับไมโครวินาที ทำให้สแต็กจังหวะสัญญาณบลูทูธสูญเสียการซิงโครไนซ์ (Time Slot Miss) และเกิดปัญหาการหลุดการเชื่อมต่ออยู่บ่อยครั้ง (Connection Dropout) การย้ายการประมวลผลข้อมูลและข้อมูลพจนานุกรมประโยคดีบักลงมาไว้ที่ `loop()` โดยใช้ธงสถานะจึงเป็นแนวปฏิบัติที่ถูกต้องสำหรับการเขียนโปรแกรมระบบฝังตัวระดับอาชีพ

---

## 3. เนื้อหาของบทเรียนฉบับปรับปรุงใหม่ทั้งหมด (Revised Chapter 6 Contents)

เนื้อหาด้านล่างนี้คือไฟล์ `chapters/ch06-wireless/README.md` ฉบับแก้ไขข้อผิดพลาดทางวิศวกรรม เทคนิค และการแปลภาษาครบทุกประเด็นแล้ว โดยสามารถนำไปเขียนทับโครงสร้างไฟล์เดิมได้ทันที:
...
*   **ย่าน 2.4 GHz (ความถี่ 2.400 GHz ถึง 2.4835 GHz):**
    ...
    *   *ลักษณะเด่น:* มีแบนด์วิดท์ที่กว้างกว่า ส่งข้อมูลได้เร็วกว่าย่าน Sub-GHz แต่ความยาวคลื่นมีขนาดสั้น (~12.5 cm) ทำให้ถูกดูดซับโดยสิ่งกีดขวางได้ง่าย โดยเฉพาะน้ำและความชื้น (เนื่องจากโมเลกุลของน้ำเป็นโมเลกุลมีขั้ว จึงเกิดการหมุนตัวตามสนามไฟฟ้าสลับขั้วอย่างรวดเร็วและดูดซับพลังงานวิทยุในย่านความถี่นี้ได้ดีผ่านกลไกการผ่อนคลายไดอิเล็กทริก (Dielectric Relaxation) หรือการหมุนตัวทางไฟฟ้าเคมี (Dipole Rotation) ไม่ใช่การสั่นพ้องระดับโมเลกุล (Water Resonance Frequency) ซึ่งแท้จริงแล้วความถี่สั่นพ้องของน้ำอยู่ในย่านหลายสิบกิกะเฮิรตซ์ (THz/GHz Range) อีกทั้งยังมีปัญหาการรบกวนของสัญญาณที่แออัดเนื่องจากมีอุปกรณ์ใช้งานหนาแน่น
...
สูตรทางคณิตศาสตร์ที่ใช้อธิบายการลดทอนของสัญญาณวิทยุในสภาวะทางอุดมคติคำนวณจากความสูญเสียระหว่างสายอากาศรับและส่งที่เป็นแบบรอบทิศทางสมบูรณ์ (Isotropic Antennas ซึ่งมีกำลังขยาย 0 dBi) เขียนความสูญเสียในเส้นทาง (Path Loss) ได้ดังนี้:
...
เมื่อต้องการคำนวณกำลังสัญญาณฝั่งรับจริง (Received Power: $P_r$) ภายใต้ผลของอัตราขยายสายอากาศส่ง ($G_t$) และสายอากาศรับ ($G_r$) ตามสมการส่งผ่านของฟรีส (Friis Transmission Equation) หรือการคำนวณงบประมาณลิงก์ (Link Budget) จะคำนวณได้แยกออกต่างหากดังนี้:
$$P_r\text{ (dBm)} = P_t\text{ (dBm)} + G_t\text{ (dBi)} + G_r\text{ (dBi)} - \text{FSPL (dB)}$$
โดยที่ $P_t$ คือกำลังส่งสัญญาณจากฝั่งส่งในหน่วย dBm และกำลังขยายสายอากาศมีหน่วยเป็น dBi
...
ตัวอย่างโค้ดด้านล่างแสดงการออกแบบการเชื่อมต่อ Wi-Fi ที่ดีที่สุดสำหรับอุตสาหกรรม โดยใช้หลักการทำงานแบบ **Non-blocking** (การประมวลผลระบบโดยไม่ปล่อยให้ฟังก์ชันรอ Wi-Fi ไปบล็อกการทำงานหลัก เช่น การอ่านค่าเซ็นเซอร์อุณหภูมิมอเตอร์) การตั้งค่า Static IP และปรับปรุงความเสถียรเพื่อป้องกัน Watchdog Reset ด้วยการคืนเวลาประมวลผล รวมถึงจำกัดเงื่อนไขการเช็กเว็บเซิร์ฟเวอร์เฉพาะเมื่อ Wi-Fi เชื่อมต่อสำเร็จ
...
void loop() {
  ...
  // ป้องกันการเรียกใช้ server.available() เปล่าประโยชน์ขณะไม่ได้เชื่อมต่อ Wi-Fi
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (client) {
      ...
      while (client.connected() && (millis() - clientConnectedTime < clientTimeoutLimit)) {
        if (client.available()) {
          ...
          delay(1); // ป้องกัน Task Watchdog Starvation โดยการปล่อยให้ FreeRTOS ทำงานอื่น
        }
      }
...
*   การส่งข้อมูลระหว่างกันจะกระทำโดยตรงผ่านหน้าต่างส่งที่มีการขยับและกวาดความถี่ (Frequency Hopping) ครอบคลุมทั้ง 37 ช่องที่เหลือเพื่อหลีกเลี่ยงการโดนสัญญาณคลื่นวิทยุตัวอื่นมารบวนและเพิ่มความปลอดภัยของข้อมูล

---

### 6.3.4 ตัวอย่างโค้ด ESP32 BLE GATT Server (Environmental Service)

โค้ด Arduino C++ สำหรับรันบน ESP32 เพื่อทำหน้าที่เป็น BLE GATT Server ทำการแพร่กระจายสัญญาณโฆษณา รับส่งข้อมูลค่าเซ็นเซอร์อุณหภูมิสิ่งแวดล้อม และรับคำสั่งควบคุมระบบ โค้ดได้รับการปรับปรุงให้ปลอดภัยข้ามเธรดด้วยคำสั่ง `volatile` ย้ายงานพิมพ์พอร์ตซีเรียลที่บล็อกเวลาออกจากฟังก์ชัน Callback และแก้ไขข้อผิดพลาดอาร์กิวเมนต์ตัวรับการโฆษณา
...
// ตัวแปรเชื่อมต่อใช้ volatile เพื่อความปลอดภัยจาก Optimization ของคอมไพเลอร์ข้ามเธรด
volatile bool isClientConnected = false;
volatile bool wasClientConnected = false;
unsigned long prevNotifyTime = 0;
const unsigned long notifyPeriod = 3000; // ส่ง Notify อัปเดตทุก 3 วินาที

// ตัวแปรและธงรับคำสั่งควบคุมเพื่อย้ายงานดีบัก Serial ออกจากสภาวะ Callback
volatile bool commandReceived = false;
std::string latestCommand = "";

// Callback สำหรับรับรู้การต่อเข้า/หลุดออกของ Client (กำหนดเป็น public)
...
// Callback สำหรับรอรับการเขียนข้อมูลเปลี่ยนสถานะ (Write) จาก Client (กำหนดเป็น public)
class CommandReceiverCallbacks: public BLECharacteristicCallbacks {
  public:
    void onWrite(BLECharacteristic *pCharacteristic) {
      std::string receivedData = pCharacteristic->getValue();
      if (receivedData.length() > 0) {
        latestCommand = receivedData;
...
void setup() {
  ...
  // ตั้งค่ารอบเวลาโฆษณาที่เหมาะสมเพื่อช่วยประหยัดแบตเตอรี่และเชื่อมโยงได้เสถียร
  pAdvertising->setMinPreferred(0x06); // ช่วยแก้ปัญหาการเชื่อมต่อกับอุปกรณ์ iOS
  pAdvertising->setMaxPreferred(0x12); // แก้ไขจากคำสั่งซ้ำซ้อนเดิม
...
void loop() {
  ...
  // 1. ดีบักสถานะการเชื่อมต่ออย่างปลอดภัยภายใน Thread หลัก (loop)
  ...
  // 2. กลไกจัดการเริ่มสัญญาณ Advertising ใหม่เมื่อ Client ตัดสัมพันธ์ (Auto-Restart Advertising)
  if (!isClientConnected && wasClientConnected) {
  ...
  // 3. หาก Client ต่ออยู่ ให้อัปเดตค่าอุณหภูมิส่งออกไปแบบ Notify
  if (isClientConnected) {
    if (now - prevNotifyTime >= notifyPeriod) {
      ...
      // ส่งข้อมูลขนาด 2 ไบต์ไปยัง Characteristic (จัดเก็บในรูปแบบไบต์อาร์เรย์ Little-Endian)
  ...
  // 4. ตรวจจับการเขียนข้อมูลคำสั่งมาจากช่องทาง Characteristic (onWrite) ในเธรดหลัก
  if (commandReceived) {
    commandReceived = false; // เคลียร์สถานะธง
    
    Serial.println("\n----------------------------------------");
    Serial.print("[BLE Control] ได้รับคำสั่งสั่งการ: ");
    Serial.println(latestCommand.c_str());
    
    // เปรียบเทียบคำสั่ง
    if (latestCommand == "ACTIVATE") {
      Serial.println("[BLE Control] ผลลัพธ์: เปิดทำงานระบบวาล์วน้ำหลัก");
    } else if (latestCommand == "DEACTIVATE") {
...
2.  **Zigbee Router (ZR - ตัวสลับและส่งต่อ):**
    *   ทำหน้าที่เป็นตัวผ่านกระจายสัญญาณและสลับหาเส้นทางข้อมูล (Routing) เพื่อส่งต่อข้อมูลข้ามทอดไปยังอุปกรณ์อื่น
...
*   **Spreading Factor (SF):** ค่าปัจจัยการคูณขยายความกว้างของรหัส (มีค่าตั้งแต่ SF7 ถึง SF12)
    *   *SF ต่ำ (SF7):* ข้อมูลส่งได้รวดเร็ว ใช้เวลาอยู่ในอากาศสั้น (Short Time-on-Air) แต่อุปกรณ์รับต้องการระดับสัญญาณที่แรงกว่า (ต้องการอัตราส่วนสัญญาณต่อสัญญาณรบกวนหรือ SNR ที่สูงกว่าในการถอดรหัส) เหมาะสำหรับระยะใกล้
    *   *SF สูง (SF12):* ทำให้อุปกรณ์รับมีความไวต่อสัญญาณสูงสุด (Receiver Sensitivity) สามารถถอดรหัสสัญญาณที่อ่อนมากหรือติดลบจมอยู่ในคลื่นรบกวนได้ดีที่สุด (SNR ต่ำสุดถึง -20 dB) ส่งผลให้ข้อมูลเดินทางได้ระยะไกลมากเป็นพิเศษและทนทานสูงมาก แต่มีข้อเสียคือความเร็วข้อมูลต่ำสุด ใช้เวลาในอากาศนานสุด (Long Time-on-Air) และสิ้นเปลืองพลังงานแบตเตอรี่ในอุปกรณ์มากกว่า
...
3.  **Network Server (แม่ข่ายควบคุมระบบ):** ทำหน้าที่ส่วนการคำนวณถอดรหัส ตรวจสอบความปลอดภัย ขจัดข้อมูลซ้ำซ้อนในกรณีมีเกตเวย์หลายตัวได้รับสัญญาณพร้อมกัน และควบคุมอัตราการส่งข้อมูลและกำลังส่งแบบปรับตัว (Adaptive Data Rate - ADR) เพื่อช่วยยืดอายุการใช้งานแบตเตอรี่
...
*   **กลไกความแรงและการเจาะทะลวงของคลื่น (Deep Indoor Coverage):** สามารถปรับเพิ่มขีดความสามารถการครอบคลุมสัญญาณ (Coverage Enhancement) หรือเพิ่มระดับงบประมาณความสูญเสียในลิงก์ (Link Margin) ให้บอร์ดได้สูงสุดถึง +20 dB เมื่อเทียบกับเครือข่ายเซลลูลาร์ทั่วไป จึงเป็นหนึ่งในทางเลือกสำหรับงานอ่านข้อมูลในชั้นใต้ดิน ผนังหนา หรือท่อน้ำประปา
