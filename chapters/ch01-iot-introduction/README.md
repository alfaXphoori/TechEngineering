...
<details>
<summary>📋 แนวเฉลยแบบฝึกหัดท้ายบท (ข้อ 1-5) (กดเพื่อขยาย — สำหรับผู้สอนเท่านั้น)</summary>

### แนวเฉลยแบบฝึกหัดท้ายบทที่ 1 (ข้อ 1-5)

**1. จงอธิบายความหมายของ IoT และยกตัวอย่างอุปกรณ์ IoT ที่นักศึกษาใช้ในชีวิตประจำวันมาอย่างน้อย 3 อย่าง พร้อมระบุว่าแต่ละอุปกรณ์ใช้เซนเซอร์ชนิดใด**
*   **แนวคำตอบ:** IoT (Internet of Things) คือ เครือข่ายของวัตถุทางกายภาพ (Things) ที่มีการติดตั้งเซนเซอร์ ซอฟต์แวร์ และเทคโนโลยีในการเชื่อมต่อแลกเปลี่ยนข้อมูลกับระบบหรืออุปกรณ์อื่น ๆ ผ่านเครือข่ายอินเทอร์เน็ตโดยอัตโนมัติโดยไม่จำเป็นต้องใช้มนุษย์ควบคุมโดยตรง
*   **ตัวอย่างอุปกรณ์และเซนเซอร์:**
    1.  **นาฬิกาอัจฉริยะ (Smart Watch):** เซนเซอร์ตรวจวัดการเต้นของหัวใจแบบแสง (PPG) และเซนเซอร์วัดการเคลื่อนไหว (Accelerometer/Gyroscope)
    2.  **เครื่องปรับอากาศอัจฉริยะ (Smart AC):** เซนเซอร์วัดอุณหภูมิและความชื้น (Thermistor)
    3.  **หุ่นยนต์ดูดฝุ่น (Robot Vacuum Cleaner):** เซนเซอร์วัดระยะทางอินฟราเรด/LiDAR และสวิตช์ชน (Bumper Switch)

**2. จงเปรียบเทียบความแตกต่างระหว่าง M2M กับ IoT มาอย่างน้อย 3 ประเด็น โดยใช้ตารางประกอบ**
*   **แนวคำตอบ:** 
    | ประเด็นเปรียบเทียบ | Machine-to-Machine (M2M) | Internet of Things (IoT) |
    | :--- | :--- | :--- |
    | **โครงสร้างสถาปัตยกรรม** | เน้นการเชื่อมต่อแบบจุดต่อจุด (Point-to-Point) สื่อสารกันเองระหว่างคู่เครื่องจักร | บูรณาการผ่านระบบคลาวด์ มีสถาปัตยกรรมแบบกระจายเชื่อมต่อหลายระบบพร้อมกัน |
    | **การเชื่อมต่อและโปรโตคอล** | มักใช้อินเทอร์เน็ตแบบปิด (Private Network) หรือโครงข่ายโทรคมนาคมเฉพาะ (Cellular/Proprietary Link) | ทำงานบนมาตรฐานอินเทอร์เน็ตแบบเปิด (IP-based Protocols เช่น HTTP, MQTT) |
    | **การบูรณาการข้อมูล** | เก็บข้อมูลในถังข้อมูลเฉพาะเจาะจงของระบบปิด (Siloed Data) ข้อมูลไม่นำไปประมวลผลต่อ | ส่งข้อมูลขึ้นคลาวด์เพื่อนำไปวิเคราะห์ร่วมกับ Big Data, Web App หรือ AI/ML |

**3. วาดแผนภาพสถาปัตยกรรม IoT 4 ชั้น สำหรับระบบตรวจสอบอุณหภูมิของเตาอบอุตสาหกรรม โดยระบุเทคโนโลยีที่ใช้ในแต่ละชั้น**
*   **แนวคำตอบ:**
    1.  **Perception Layer:** เทอร์โมคัปเปิลชนิด K (Type K Thermocouple), IC แปลงสัญญาณ MAX31855, ไมโครคอนโทรลเลอร์ ESP32
    2.  **Network Layer:** สัญญาณเชื่อมต่อไร้สาย Wi-Fi หรือ LAN และโปรโตคอลการรับส่งข้อความ MQTT (Publish/Subscribe)
    3.  **Processing Layer:** MQTT Broker (เช่น Mosquitto), บริการคลาวด์ ThingsBoard Cloud หรือ InfluxDB สำหรับบันทึกข้อมูล Time-series
    4.  **Application Layer:** Real-time Dashboard แสดงกราฟอุณหภูมิบนหน้าเว็บ ThingsBoard หรือ Grafana, ส่ง LINE Notify/Email แจ้งเตือนอุณหภูมิผิดปกติ

**4. หากนักศึกษาต้องออกแบบระบบ IoT เพื่อตรวจสอบสภาพตลับลูกปืนของมอเตอร์ในโรงงาน จงระบุ: (ก) เซนเซอร์ที่ต้องใช้ พร้อมเหตุผล (ข) ไมโครคอนโทรลเลอร์ที่เหมาะสม (ค) โปรโตคอลสื่อสารที่เลือกใช้ พร้อมเหตุผล (ง) แพลตฟอร์มคลาวด์ที่เหมาะสม**
*   **แนวคำตอบ:**
    *   **(ก) เซนเซอร์:** เซนเซอร์วัดการสั่นสะเทือน (Vibration Sensor/Accelerometer เช่น ADXL345) เพื่อตรวจจับความผิดปกติของตลับลูกปืนขณะหมุน และเซนเซอร์วัดอุณหภูมิแบบไร้สัมผัส (Infrared Temperature Sensor เช่น MLX90614) เพื่อวัดความร้อนสะสมจากการเสียดสี
    *   **(ข) ไมโครคอนโทรลเลอร์:** ESP32 เนื่องจากมีหน่วยประมวลผลที่เร็วพอสามารถวิเคราะห์ข้อมูลการสั่นเชิงความถี่ (FFT) ได้ที่อุปกรณ์ปลายทาง (Edge processing) และมีตัวเชื่อม Wi-Fi ในตัว
    *   **(ค) โปรโตคอลการสื่อสาร:** MQTT เนื่องจากมีความรวดเร็ว แบนด์วิดท์ต่ำ (Lightweight) เหมาะกับการส่งข้อมูลถี่ ๆ แบบเรียลไทม์ในโรงงาน
    *   **(ง) แพลตฟอร์มคลาวด์:** ThingsBoard Cloud หรือ InfluxDB/Grafana Stack เพื่อเก็บข้อมูลแบบอนุกรมเวลา (Time-series) และสร้างแดชบอร์ดติดตามสุขภาพและแจ้งเตือนวิศวกรซ่อมบำรุง
...
*   **แนวคำตอบ:**
    1.  **ระบบควบคุมความร้อนและระบายอากาศอัจฉริยะ (Smart HVAC):** ใช้เซนเซอร์วัดอุณหภูมิ ความชื้น และระดับก๊าซ CO2 ส่งข้อมูลไปประมวลผลเพื่อควบคุมรอบมอเตอร์พัดลมผ่าน VFD และปรับการเปิดวาล์วน้ำเย็น (Chilled Water Valve) ช่วยลดพลังงาน
    2.  **ระบบตรวจจับการรั่วไหลของของไหลในท่อส่งน้ำ/ก๊าซ (Smart Pipeline Leakage Detection):** ใช้เซนเซอร์วัดแรงดัน (Pressure Sensor) และอัตราการไหล (Flow Rate Sensor) ตามจุดต่าง ๆ เพื่อตรวจสอบและสั่งปิดวาล์วไฟฟ้า (Solenoid Valve) อัตโนมัติเมื่อพบแรงดันตกกระทันหัน
    3.  **ระบบติดตามสภาพเครื่องจักรหมุนและมอเตอร์ไฟฟ้า (Rotating Machinery Monitoring):** ใช้เซนเซอร์สั่นสะเทือนร่วมกับเซนเซอร์กระแสไฟฟ้า (Current Transformer - CT) และวัดรอบหมุน (Tachometer) เพื่อคำนวณประสิทธิภาพทางกลแบบเรียลไทม์ และคาดการณ์เวลาซ่อมบำรุงล่วงหน้า
