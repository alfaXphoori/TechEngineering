...
flowchart TD
    ...
    %% Telemetry Flow (Upwards)
    PhysicalEnv["🌍 โลกกายภาพ"]:::physical
    Sensor["📡 Perception Layer<br>(ADC / I2C)"]:::device
    Network["🌐 Network Layer<br>(JSON / MQTT)"]:::network
    Cloud["☁️ Cloud Layer<br>(InfluxDB / Processing)"]:::cloud
    App["💻 Application Layer<br>(Dashboard / Alert)"]:::app

    PhysicalEnv -- "〰️ สัญญาณอนาล็อก" --> Sensor
    Sensor -- "📦 ตัวแปร / Float" --> Network
    Network -- "📶 IP Packets" --> Cloud
    Cloud -- "🔗 REST API / WebSockets" --> App

    %% Control Flow (Downwards)
    Gateway["🔌 Network Gateway"]:::network
    Actuator["⚙️ Actuator Layer<br>(Relay / Valve)"]:::device

    App -- "✉️ Control Topic (MQTT/JSON)" --> Gateway
