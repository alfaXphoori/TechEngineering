...
<svg viewBox="0 0 760 220" width="100%" height="auto" xmlns="http://www.w3.org/2000/svg" font-family="'IBM Plex Sans Thai', system-ui, sans-serif">
  ...
  <text x="120" y="80" class="text-title" text-anchor="middle">Arduino Uno (MCU)</text>
...
## 4.2 ประเภทของตัวกระทำ (Types of Actuators)
...
### 4.2.2 รายละเอียดแต่ละชนิด

**LED และบัซเซอร์** — โหลดเบา สามารถขับจาก GPIO ของ Arduino Uno ได้โดยตรง (กระแส ≤ 20 mA ต่อขา) เหมาะสำหรับฝึกพื้นฐาน
...
### 4.2.3 หลักการทางฟิสิกส์ของมอเตอร์ไฟฟ้ากระแสตรง (DC Motor Physics)
...
## 4.3 การขับโหลด (Driving Loads)

### 4.3.1 ทำไม MCU ขับโหลดใหญ่โดยตรงไม่ได้?

ขา GPIO ของ Arduino Uno จ่ายกระแสได้อย่างปลอดภัยสูงสุดประมาณ **~20 mA ต่อขา** (และกระแสรวมทุกขาไม่ควรเกิน ~200 mA) ซึ่ง:
...
> 💡 **ข้อควรระวังเรื่องกระแส:** การต่อโหลดที่กินกระแสเกินพิกัดเข้ากับ GPIO โดยตรงจะทำให้ชิป Arduino Uno เสียหายถาวร ทุกครั้งที่เลือกตัวกระทำ ให้ตรวจสอบกระแสที่โหลดต้องการก่อน หากเกิน 20 mA ให้ใช้วงจรขับเสมอ

### 4.3.2 วงจรขับด้วยทรานซิสเตอร์ / MOSFET
...
<svg viewBox="0 0 760 320" width="100%" height="auto" xmlns="http://www.w3.org/2000/svg" font-family="'IBM Plex Sans Thai', system-ui, sans-serif">
  ...
  <style>
    ...
    .state-low { animation: glowLow 6s ease-in-out infinite; }
  ...
  <!-- Wires -->
  <!-- Base Circuit Wire -->
  <path d="M 120 190 L 370 190" class="wire"/>
  <!-- Collector/Load Wire -->
  <path d="M 380 50 L 380 150" class="wire"/>
  <!-- Emitter/GND Wire -->
  <path d="M 380 230 L 380 270" class="wire"/>
  
  <!-- Animated Currents -->
  <path d="M 120 190 L 370 190 M 370 207 L 380 230 L 380 270" class="base-current"/>
  <path d="M 380 50 L 380 150 L 370 173 M 370 207 L 380 230 L 380 270" class="load-current"/>
  
  <!-- Arduino Uno Pin representation -->
  <rect x="25" y="150" width="95" height="80" class="mcu"/>
  <text x="72.5" y="178" class="text-main" fill="#7c3aed" text-anchor="middle">Arduino GPIO</text>
  <g class="state-low">
    <text x="72.5" y="205" class="text-code" fill="#64748b" text-anchor="middle">0V (LOW)</text>
  </g>
  <g class="state-high">
    <text x="72.5" y="205" class="text-code" fill="#10b981" text-anchor="middle">5.0V (HIGH)</text>
  </g>
  ...
  <g transform="translate(0,0)">
    ...
    <line x1="370" y1="173" x2="370" y2="207" stroke="#334155" stroke-width="3.5" stroke-linecap="round"/>
    <line x1="330" y1="190" x2="370" y2="190" stroke="#334155" stroke-width="2.5"/>
    <line x1="380" y1="150" x2="370" y2="173" stroke="#334155" stroke-width="2.5"/>
    <line x1="370" y1="207" x2="380" y2="230" stroke="#334155" stroke-width="2.5"/>
    <polygon points="379,227 378,216 372,220" fill="#334155"/>
  </g>
  <text x="415" y="244" class="text-main">NPN Transistor</text>
  <text x="415" y="262" class="text-code" fill="#4b5563">(e.g., 2N2222)</text>
  <line x1="374" y1="165" x2="386" y2="172" stroke="#94a3b8" stroke-width="1"/>
  <line x1="375" y1="216" x2="386" y2="201" stroke="#94a3b8" stroke-width="1"/>
  <line x1="370" y1="190" x2="374" y2="191" stroke="#94a3b8" stroke-width="1"/>
  <text x="375" y="193" class="text-code" fill="#475569">B</text>
  <text x="389" y="177" class="text-code" fill="#475569">C</text>
  <text x="389" y="206" class="text-code" fill="#475569">E</text>
...
> ⚠️ **แนะนำให้ใช้ MOSFET แบบ "Logic-Level" (เช่น IRLZ44N, AOD4184):** แม้ว่าบอร์ด Arduino Uno จะจ่ายแรงดันเอาต์พุตได้ 5 V ซึ่งสูงกว่าบอร์ด 3.3 V แต่ MOSFET กำลังทั่วไป (เช่น IRF540) ยังคงต้องการแรงดัน Gate-Source ($V_{GS}$) สูงถึง 10 V เพื่อให้นำกระแสได้สมบูรณ์และได้ค่าความต้านทาน $R_{DS(on)}$ ต่ำสุด หากแรงดันที่ป้อนให้เกตต่ำเกินไป MOSFET จะทำงานในย่าน Linear เกิดความต้านทานและมีความร้อนสะสมสูงมากจนพังได้ การเลือกเบอร์ Logic-Level (ที่มี $V_{GS(th)}$ ต่ำราว 1–2 V) จึงปลอดภัยและให้ประสิทธิภาพสูงสุดสำหรับไมโครคอนโทรลเลอร์
...
### 4.3.3 โมดูลรีเลย์ (Relay Module)
...
### 4.3.4 มอเตอร์ไดรเวอร์ (Motor Driver)
...
- ENA → Pin 3 (PWM ควบคุมความเร็ว)
- IN1, IN2 → Pin 2, Pin 4 (ควบคุมทิศทาง)
- OUT1, OUT2 → มอเตอร์ DC
- VCC → แหล่งจ่ายไฟภายนอกสำหรับมอเตอร์ (เช่น ถ่าน 9V หรือกล่องถ่าน)
- GND → **ต้องต่อร่วมกัน** ระหว่างแหล่งจ่ายไฟภายนอกและบอร์ด Arduino Uno

**ตัวอย่างโค้ด Arduino Uno — ควบคุมความเร็วและทิศทางมอเตอร์ DC ผ่าน L298N:**

```cpp
// Arduino Uno + L298N : ควบคุมมอเตอร์ DC 1 ตัว (ความเร็ว + ทิศทาง)
const int IN1 = 2;   // สัญญาณทิศทาง A ต่อ Pin 2
const int IN2 = 4;   // สัญญาณทิศทาง B ต่อ Pin 4
const int ENA = 3;   // สัญญาณ ENA ต่อ Pin 3 (PWM)
...
// speed: -255 (ถอยหลังเต็มที่) ... 0 (หยุด) ... +255 (เดินหน้าเต็มที่)
void driveMotor(int speed) {
  ...
  analogWrite(ENA, abs(speed));  // ส่งสัญญาณ PWM เพื่อควบคุมความเร็วรอบ (0-255)
}

void loop() {
  driveMotor(180);   // เดินหน้าด้วยความเร็ว ~70%
  delay(2000);
  driveMotor(0);     // หยุดเบรก 1 วินาที
  delay(1000);
  driveMotor(-180);  // ถอยหลังด้วยความเร็ว ~70%
  delay(2000);
  driveMotor(0);     // หยุดเบรก 1 วินาที
  delay(1000);
}
...
## 4.4 การป้องกันวงจร (Circuit Protection)

### 4.4.1 ไดโอด Flyback (Flyback Diode / Freewheeling Diode)
...
### 4.4.2 การแยกกราวด์ร่วม (Common Ground)

เมื่อใช้แหล่งจ่ายหลายตัว (เช่น USB สำหรับ Arduino Uno + อะแดปเตอร์ 12 V สำหรับมอเตอร์) **GND ของทุกแหล่งจ่ายต้องต่อร่วมกัน** มิฉะนั้นสัญญาณจะไม่มีจุดอ้างอิงร่วม → วงจรทำงานไม่ถูกต้องหรือเสียหาย

### 4.4.3 ออปโตคัปเปลอร์ (Optocoupler / Optoisolator)
...
## 4.5 PWM กับการควบคุมตัวกระทำ (PWM & Actuator Control — Introduction)
...
**ความละเอียด (Resolution):** Arduino Uno กำหนดความละเอียด PWM เป็นจำนวนบิตได้ ความละเอียด $n$ บิตให้จำนวนระดับ Duty ที่ตั้งได้เท่ากับ $2^n$ ระดับ เช่น 8 บิต = 256 ระดับ (ค่า 0–255), 10 บิต = 1024 ระดับ ยิ่งบิตมากยิ่งปรับละเอียด แต่ความถี่สูงสุดที่ทำได้จะลดลง (มีความสัมพันธ์ผกผันกับความละเอียดผ่านคล็อกภายในของตัวจับเวลา)
...
**ตัวอย่างโค้ด Arduino Uno — ควบคุมความสว่าง LED ด้วย PWM (ทดลองบน Tinkercad Circuits):**

```cpp
// Arduino Uno LED Brightness Control — PWM
// ทดสอบบน Tinkercad Circuits: ต่อ LED + R 220Ω ที่ Pin 9 (PWM)

const int ledPin = 9; // ใช้ Pin ที่มีสัญลักษณ์ ~ (3, 5, 6, 9, 10, 11)
...
void loop() {
  ...
  for (int duty = 0; duty <= 255; duty += 5) {
    analogWrite(ledPin, duty); // ค่า 0-255 (0% - 100% Duty Cycle)
    delay(30);
  }
  // ค่อย ๆ หรี่ลง
...
## 4.6 การเลือกวงจรขับให้เหมาะกับโหลด (Selecting the Right Driver)
...
## 4.7 เซอร์โวมอเตอร์เชิงลึก (Servo Motor in Depth)

### 4.7.1 โครงสร้างภายในและหลักการควบคุมตำแหน่ง
...
### 4.7.2 การเข้ารหัสมุมด้วยความกว้างพัลส์ (Pulse-Width Encoding)
...
**ตัวอย่างโค้ด Arduino Uno — ควบคุมมุมเซอร์โว (ใช้ไลบรารี standard Servo.h):**

```cpp
#include <Servo.h>

Servo myServo;
const int servoPin = 9; // ต่อขาสัญญาณเซอร์โวเข้ากับ Pin 9 ของ Arduino Uno

void setup() {
  myServo.attach(servoPin); // เริ่มต้นเซอร์โวมอเตอร์ (ใช้ความกว้างพัลส์มาตรฐาน 544-2400 µs)
}

void loop() {
  myServo.write(0);     delay(1000);     // สั่งหมุนไปยังมุม 0 องศา
  myServo.write(90);    delay(1000);     // สั่งหมุนไปยังมุม 90 องศา
  myServo.write(180);   delay(1000);     // สั่งหมุนไปยังมุม 180 องศา

  // กวาดมุมอย่างช้าๆ (Sweep 0-180 องศา)
...
## 4.8 สเต็ปเปอร์มอเตอร์เชิงลึก (Stepper Motor in Depth)

### 4.8.1 หลักการหมุนทีละสเต็ป
...
### 4.8.2 รูปแบบการขับ (Drive Modes)
...
**ตัวอย่างโค้ด Arduino Uno — ขับสเต็ปเปอร์ผ่าน A4988 (STEP/DIR):**

```cpp
const int STEP_PIN = 5;         // ต่อขาสัญญาณพัลส์เข้า Pin 5
const int DIR_PIN  = 4;         // ต่อสัญญาณทิศทางเข้า Pin 4
const int STEPS_PER_REV = 200;  // จำนวนสเต็ปต่อรอบ (เช่น มอเตอร์ 1.8 องศาต่อสเต็ป)
...
void rotateSteps(int steps, bool cw) {
  digitalWrite(DIR_PIN, cw ? HIGH : LOW); // ตั้งลอจิกทิศทาง
  for (int i = 0; i < steps; i++) {
    digitalWrite(STEP_PIN, HIGH);
    delayMicroseconds(800);        // ยิ่งคาบเวลาพัลส์สั้น มอเตอร์ยิ่งหมุนเร็วขึ้น
...
void loop() {
  rotateSteps(STEPS_PER_REV, true);   // สั่งหมุน 1 รอบตามเข็มนาฬิกา
  delay(1000);
  rotateSteps(STEPS_PER_REV, false);  // สั่งหมุน 1 รอบทวนเข็มนาฬิกา
  delay(1000);
}
```
...
- Arduino Uno จ่ายกระแสจาก GPIO ได้จำกัด (~20 mA) จึงต้องใช้ **วงจรขับ** (ทรานซิสเตอร์, MOSFET, โมดูลรีเลย์, มอเตอร์ไดรเวอร์) สำหรับโหลดที่กินกระแสมาก โดยคำนวณค่า $R_B$ ของทรานซิสเตอร์จากสภาวะอิ่มตัว และใช้ MOSFET แบบ **logic-level** กับ Arduino Uno
...
**ข้อ 2:** มอเตอร์ DC ตัวหนึ่งใช้แรงดัน 12 V กินกระแส 800 mA ถ้านักศึกษาต่อ GPIO ของ Arduino Uno เข้ากับมอเตอร์โดยตรง จะเกิดอะไรขึ้น? ควรใช้วงจรขับแบบใด?

**ข้อ 3:** จงวาดวงจรขับรีเลย์ 5 V ด้วยทรานซิสเตอร์ NPN โดยมีองค์ประกอบครบถ้วน (ตัวต้านทาน Base, ไดโอด Flyback, แหล่งจ่าย, Arduino GPIO) พร้อมระบุค่าอุปกรณ์
