...
```
    ┌──────────┐
 ──►│  Fetch   │ ดึงคำสั่งจากหน่วยความจำ (ระบุโดย PC) เข้ามาเก็บใน IR
    └────┬─────┘
         ▼
    ┌──────────┐
    │  Decode  │ ถอดรหัส opcode ใน IR เพื่อดูว่าเป็นคำสั่งอะไรและต้องใช้ข้อมูลใด
    └────┬─────┘
         ▼
    ┌──────────┐
    │ Execute  │ ส่งสัญญาณควบคุมให้ ALU หรือส่วนที่เกี่ยวข้องทำการประมวลผลจริง
    └────┬─────┘
         ▼
    ┌──────────┐
    │Writeback │ เขียนผลลัพธ์ที่ได้กลับลงใน Register หรือหน่วยความจำข้อมูล
    └────┬─────┘
         │
         └─────── เพิ่มค่า PC เพื่อไปคำสั่งถัดไป (หรือกระโดดไปยังแอดเดรสใหม่หากเกิดการ Jump/Branch)
```
...
ESP32 (รุ่น WROOM-32) ใช้ตัวประมวลผลกลาง 32 บิต สถาปัตยกรรม **Xtensa LX6** จำนวน 2 แกน (Dual-core) ทำงานขนานกันภายใต้การบริหารจัดการทรัพยากรแบบ Symmetric Multiprocessing (SMP) ของ FreeRTOS
...
ESP32 มีการแบ่งหน่วยความจำภายใน 520 KB SRAM และเชื่อมต่อภายนอกผ่าน Memory Management Unit (MMU) ดังรายละเอียด:

##### 1) หน่วยความจำภายใน (Internal Memory)
- **SRAM 0 (ขนาด 192 KB):** มักจะถูกใช้ในลักษณะ **Instruction RAM (IRAM)** สำหรับประมวลผลโค้ดที่ต้องการรันด้วยความเร็วสูงมาก เช่น Interrupt Service Routines (ISR) หรือฟังก์ชันสำคัญของระบบปฏิบัติการ
- **SRAM 1 (ขนาด 128 KB):** สามารถเข้าถึงได้ทั้งเป็น IRAM หรือ Data RAM (DRAM) และทำหน้าที่เป็นสะพานสำหรับการรับส่งข้อมูลความเร็วสูงผ่าน DMA (Direct Memory Access)
- **SRAM 2 (ขนาด 200 KB):** ใช้เป็น **Data RAM (DRAM)** เป็นหลัก ทำหน้าที่เก็บตัวแปรโกลบอล (Global variables) เก็บข้อมูลสแต็ก (Stack) และฮีป (Heap) สำหรับตัวแปรที่มีการจองพื้นที่แบบ Dynamic
- **Internal ROM (ขนาด 448 KB):** บันทึกโปรแกรม Bootloader, ฟังก์ชันระบบปฏิบัติการ FreeRTOS และฟังก์ชันไลบรารีพื้นฐานอื่น ๆ (เช่น ฟังก์ชันทางคณิตศาสตร์, คริปโตกราฟี) ที่มาจากผู้ผลิตชิป

##### 2) การเข้าถึง Flash Memory และ PSRAM ภายนอกผ่าน MMU
เนื่องจากโมดูล ESP32 เก็บโปรแกรมหลักไว้ใน External SPI Flash (เช่น 4 MB บนชิป ESP32-WROOM-32) และอาจมีการเชื่อมต่อหน่วยความจำแบบเขียน-อ่านชั่วคราวภายนอกเพิ่มเติม (External PSRAM) ผ่านการเชื่อมต่อ SPI:
- CPU จะไม่สามารถเข้าถึงข้อมูลบนชิป SPI ภายนอกได้โดยตรงในสเกลเวลาของสัญญาณนาฬิกา
- ESP32 แก้ปัญหานี้โดยใช้ **MMU (Memory Management Unit)** ในการทำ Virtual Address Mapping ดึงช่วงที่อยู่ใน Flash/PSRAM ภายนอกเข้ามาเป็นส่วนหนึ่งของ Address Space ของ CPU
- มีระบบ **Cache (ขนาด 32 KB ต่อแกน CPU)** ทำหน้าที่คอยดึงข้อมูลและคำสั่งจากหน่วยความจำภายนอกมาพักไว้ล่วงหน้า ทำให้การดึงข้อมูลจาก Flash มีความเร็วสูงเทียบเท่าการอ่านจาก SRAM ภายในตัวชิป

---

#### 4.2.4 ขากำหนดโหมดการบูตเริ่มต้น (Boot Strapping Pins)
...
#### 4.2.5 การควบคุม GPIO ในระดับรีจิสเตอร์ (GPIO Register Control)
