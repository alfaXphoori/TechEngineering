...
**เครื่องมือ:** Wokwi Simulator + ESP32 DevKit V1
**เวลา:** 3 ชั่วโมง
...
- เขียนโปรแกรม ESP32 เพื่อควบคุมเอาต์พุตดิจิทัล (Digital Output) ด้วยฟังก์ชัน `digitalWrite()` และ PWM
- ออกแบบรูปแบบการกระพริบ LED (Blink Pattern) และสร้างเสียงแจ้งเตือนด้วย Buzzer ได้
- ควบคุม Servo Motor SG90 ให้หมุนไปยังมุมที่กำหนดผ่านไลบรารี ESP32Servo
...
2. ต่อวงจร LED 3 ดวงที่ขา GPIO 2, GPIO 4 และ GPIO 5 (ต่อ Resistor 220Ω อนุกรมทุกดวง)
...
1. เพิ่ม **Piezo Buzzer** ในวงจร Wokwi ต่อที่ขา GPIO 15
...
รีเลย์ (Relay) ทำหน้าที่เป็นสวิตช์ทางอิเล็กทรอนิกส์ ใช้สัญญาณดิจิทัลจากไมโครคอนโทรลเลอร์ควบคุมการเปิด-ปิดโหลดที่ใช้ไฟฟ้าสูงกว่า เช่น หลอดไฟ AC หรือปั๊มน้ำ ใน Wokwi จะใช้ Relay Module จำลอง

### ขั้นตอนปฏิบัติ

1. เพิ่ม **Relay Module** ใน Wokwi ต่อขาสัญญาณ (Signal) ที่ GPIO 26, VCC ที่ 5V, GND ที่ GND
...
Servo Motor SG90 หมุนได้ 0°–180° ควบคุมด้วยสัญญาณ PWM ใช้ไลบรารี `ESP32Servo` บน ESP32

### ขั้นตอนปฏิบัติ

1. เพิ่ม **Servo Motor (Micro Servo)** ใน Wokwi ต่อสายสัญญาณ (Signal) ที่ GPIO 13
2. ติดตั้งไลบรารี: ใน Wokwi ให้เพิ่ม `ESP32Servo` ในไฟล์ `libraries.txt`
...
void setup() {
  Serial.begin(115200);
  myServo.attach(servoPin, 500, 2400);  // พัลส์ 500-2400 µs
...
4. สังเกตมุมการหมุนของ Servo ในหน้าจำลอง Wokwi แล้วบันทึกผล

### ตารางบันทึกผล — ส่วนที่ 4

| มุมที่สั่ง (องศา) | มุมที่สังเกตได้ใน Wokwi | ทิศทางการหมุน (CW/CCW) |
|:-:|:-:|:-:|
| 0 | | |
| 30 | | |
| 60 | | |
...
สิ่งที่ต้องส่ง:
1. **ลิงก์โปรเจกต์ Wokwi** ทั้ง 4 ส่วน (หรือรวมเป็นโปรเจกต์เดียว)
...
- [ ] ลิงก์ Wokwi เปิดได้และวงจรทำงานถูกต้อง
